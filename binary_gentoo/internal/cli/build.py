...
from ._parser import (
    add_distdir_argument_to,
    add_docker_image_argument_to,
    add_interactive_argument_to,
    add_pkgdir_argument_to,
    add_portdir_argument_to,
...
def enrich_config(config):
    ...
    if config.cpu_flags_x86 is None:
        config.cpu_flags_x86 = announce_and_check_output(
            ["portageq", "envvar", "CPU_FLAGS_X86"]
        ).rstrip()
...
def parse_command_line(argv):
    ...
    parser.add_argument(
        ...
        required=not HOST_IS_GENTOO,
        help=(
            "enforce custom CFLAGS (default: auto-detect using portageq)"
            if HOST_IS_GENTOO
            else "specify CFLAGS (required)"
        ),
    )
    parser.add_argument(
        "--cxxflags",
        ...
        required=not HOST_IS_GENTOO,
        help=(
            "enforce custom CXXFLAGS (default: auto-detect using portageq)"
            if HOST_IS_GENTOO
            else "specify CXXFLAGS (required)"
        ),
    )
    parser.add_argument(
        "--ldflags",
        ...
        required=not HOST_IS_GENTOO,
        help=(
            "enforce custom LDFLAGS (default: auto-detect using portageq)"
            if HOST_IS_GENTOO
            else "specify LDFLAGS (required)"
        ),
    )
    parser.add_argument(
        "--cpu-flags-x86",
        ...
        help=(
            (
                "enforce custom CPU_FLAGS_X86"
                " (default: auto-detect using portageq (not cpuid2cpuflags))"
            )
            if HOST_IS_GENTOO
            else "specify CPU_FLAGS_X86 (required)"
        ),
    )
...
def build(config):
    ...
    with tempfile.TemporaryDirectory() as eventual_etc_portage:
        rsync_argv = [
            "rsync",
            "--archive",
            config.host_etc_portage + "/",
            eventual_etc_portage + "/",
        ]
        ...
        if config.host_flavors_dir is not None:
            ...
            if os.path.exists(global_package_use_file_source):
                global_package_use_file_target = os.path.join(
                    eventual_etc_portage, "package.use", "ZZ-global"
                )  # to be third last in alphabetic order
                shutil.copyfile(
                    global_package_use_file_source,
                    global_package_use_file_target,
                )

            if emerge_target_type == EmergeTargetType.PACKAGE:
                flavors_yml_file = os.path.join(
                    config.host_flavors_dir,
                    category,
                    package_or_set,
                    "flavors.yml",
                )
        ...
        if common_package_use_file_content:
            common_package_use_file_target = os.path.join(
                eventual_etc_portage, "package.use", "ZZZ-common"
            )  # to be second last in alphabetic order
        ...
        for flavor in flavors_yml_doc.get("flavors", [{}]):
            ...
            single_package_use_file_target = os.path.join(
                eventual_etc_portage, "package.use", "ZZZZ-flavor"
            )  # to be last in alphabetic order
            ...
            host_log_filename = os.path.join(
                host_logdir__category__package, filename_timestamp + "-fail.log"
            )
            log_writer_process = subprocess.Popen(
                ["tee", host_log_filename], stdin=subprocess.PIPE
            )
            ...
            for step_index, step in enumerate(steps):
                ...
                package_use_content = (
                    base64.encodebytes(step.get("package.use", "").encode("ascii"))
                    .decode("ascii")
                    .replace("\n", "")
                )
                ...
                enforce_installation = (
                    config.enforce_installation
                    or not is_last_step
                    or (config.tag_docker_image is not None)
                )
